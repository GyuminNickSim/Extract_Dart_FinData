      ...
      if (startDate!="q!") {
        ...
        if (finishDate!="q!") {
          if (finishDate=="" || finishDate==as.character(format(Sys.Date(), "%Y%m%d"))) {
            DartJSONtoExcel(auth, companyCode, startDate, NULL)
          } else {
...
DartJSNOtoExcel <- function (auth, companyCode, startDate, finishDate=NULL) {
  ...
  if (as.numeric(startDate)>=20120401) {
    ...
    if (as.numeric(substr(startDate,5,8))<1001)  fjson_temp <- jsonlite::fromJSON(url_temp)
    ...
    for(i in 1:LEN) {
      ...
      } else if (as.numeric(substr(startDate,5,8))<=1001 && i>LEN-(LEN3-LEN2)) {
      ...
      if (file.size(FSdata_address)>5000 && file.exists(FSdata_address)==T) {
        ...
        for (k in 1:length(names(Transpose_IS))) {
          if (is.na(names(Transpose_IS)[k])==F) {
            if (names(Transpose_IS)[k]=="수익(매출액)" || names(Transpose_IS)[k]=="수익") {
    ...
    if (nrow(CFS_work)>5) {
      for (j in 5:nrow(CFS_work)) {
        if (substrRight(CFS_work$Date[j], 3)!="1분기") {
    ...
    if (length(subset(temp, is.null(temp)==F))>0) {
      for (ii in 1:length(subset(temp, is.null(temp)==F))) {
    ...
    if (as.numeric(substr(startDate,5,8))<1001) {
      FS_work <- subset(FS_work, index!=1)
    }
    ...
    if (as.numeric(substr(startDate,5,8))<1001) {
      IS_work <- subset(IS_work, index!=1)
    }
    ...
    if (as.numeric(substr(startDate,5,8))<1001) {
      CFS_work <- subset(CFS_work, index!=1)
    }
...
## Excution Code
##
## You can just execute StartExtract().
##########################################################################
